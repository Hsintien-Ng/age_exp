...
def generate_alias(model_cls):
    model = model_cls.__name__
    from datetime import datetime
    current_time = datetime.now().strftime('%b%d_%H-%M-%S')
    alias = '{}_{}'.format(model, current_time)
    return alias
...
# dir define
index_dir = os.path.join('/', 'home', 'xintian', 'projects', 'age_exp', 'MORPH_Small_Scale')
...
def run(model_cls, loss, predictor, calculator, train_dataset, valid_dataset, sps):
    # log setting
    alias = generate_alias(model_cls)
